...
const createMovieDetailsFilmTemplate = () => {
  return (
...
// профайл
render(siteHeaderElement, createSiteProfileTemplate(), `afterend`);

// основная разметка
render(siteMain, createBasicMarkupTemplate(), `afterbegin`);
const cardWrapper = document.querySelector(`.films-list .films-list__container`);
const cardTopRatedWrapper = document.querySelectorAll(`.films-list--extra .films-list__container`);

// Меню (фильтры и статистика)
render(siteMain, createSiteMenuTemplate(), `afterbegin`);
...
render(cardWrapper, createLoadMoreButtonTemplate(), `afterend`);

// Карточки фильма в блоке «Top rated»
// Карточки фильма в блоке «Most commented»
for (let i = 0; i < NUMBER_OF_CARDS; i++) {
  render(cardTopRatedWrapper[0], createTopRatedFilmCardTemplate(), `afterbegin`);
  render(cardTopRatedWrapper[1], createMostCommentedFilmCardTemplate(), `afterbegin`);
}
